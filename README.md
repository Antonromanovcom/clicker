...
На текущем этапе режим `inhibit` действительно предотвращает системный сон. Программа определяет платформу, ожидает заданного старта и гарантированно снимает системную блокировку при завершении. Режим `activity` пока остаётся безопасной заглушкой и не управляет вводом.
